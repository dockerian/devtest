...
buildTest() {
  ...
  echo "============================================================"
  count=0 count_bypass=0 count_failed=0 count_passed=0 count_warned=0
  ...
  local devex=`[[ "${script_source}" =~ (devtest/aft) ]] && echo "true"`
...
configDynamicArgsDict() {
  ...
  args_deleteIp="${addIpId=0xDEADBEEF}"
...
runModuleTest() {
  ...
  for ((i=1; i<=${runtimes}; i++)); do
    ...
    if [[ "${args}" =~ (0xDEADBEEF) ]]; then
      count_bypass=$((count_bypass+1))
      printf -- "%20sSkipping test ${module_name} x $i, arg: ${args}\n" " "
...
runTests() {
  ...
  echo -e "Passed: ${count_passed}, Failed: ${count_failed}, Skipped: ${count_bypass}, Warnings: ${count_warned}, Tested: ${count}\n"
  exit ${count_failed}
}
