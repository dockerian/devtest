...
#     $HP_TENANT_ID     # e.g. 10804896732690
#     $HP_IDENTITY_URL  # e.g. https://region-a.geo-1.identity.hpcloudsvc.com:35357/v2.0/
#     $HP_ACCESSKEY     # e.g. 2ADR7XRPG76J8TZ5JJ1S
#     $HP_SECRETKEY     # e.g. BXeT4Dd52uW3NHaEsfwnwYFf3c72UU1uAQyvdrzu
...
buildupEnv() {
  ...
  # if no fog repo and nor in devtest
  local clone=`[[ ! -f "Rakefile" ]] && echo "true"`
  local devex=`([[ "${PWD}" =~ (devtest/aft/fog) ]] || \
          [[ ! "${PWD}" =~ (fog) ]]) && echo "true"`
...
configTestArgs_hpcloud() {
  ...
  helion_tent_name="${HP_TENANT_NAME:=Platform-AddIn-QA}"
  helion_projectId="${HP_TENANT_ID:=10804896732690}"
  helion_accessKey="${HP_ACCESSKEY:=2ADR7XRPG76J8TZ5JJ1S}"
  helion_secretKey="${HP_SECRETKEY:=BXeT4Dd52uW3NHaEsfwnwYFf3c72UU1uAQyvdrzu}"
